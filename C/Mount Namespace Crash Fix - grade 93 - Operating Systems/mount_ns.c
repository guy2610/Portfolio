...
static struct mount_ns* allocmount_ns()
{
  ...
  for(int i=0;i<NNAMESPACE;i++)//finding an empty mount_ns entry
  {
	  mount_ns= &mountnstable.mount_ns[i];//empty mount_ns entry found
	  if(mount_ns->ref==0)
	  {
		  (mount_ns->ref)++;
		  release(&mountnstable.lock);//release lock before return
		  return (mount_ns);
	  }
  }
