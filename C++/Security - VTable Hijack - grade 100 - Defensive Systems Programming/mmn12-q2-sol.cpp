...
void handle_escape(const char* str)
{
   ...
   struct
   {
      char buffer[BUFFER_SIZE] = { 0 };////giving the buffer a size that depends on the macro
      Handler h;
   } l;
   ...
   char* p = l.buffer;
   s++;
   while (*s&& c < BUFFER_SIZE)////checking that the counter is less than the buffer size
     { *p++ = *s++;
